...
function draw(){
  ...
  text("Pontuacao: "+ pontuacao, width/2,height-500);
  ...
  if (estadoJogo === JOGAR){
    ...
    solo.x = solo.width/2;
         ...
         if(keyDown("space") && trex.y > height-60||touches.length>0&&trex.y > height-600){
    trex.velocityY = -12;
           somSalto.play();
           touches=[];
  }
...
function criarNuvens(){
  ...
  var nuvem = createSprite(width, 100, 40, 10);
    ...
    nuvem.scale = 0.5;
    nuvem.y = Math.round(random(height-3000,height-25));
    nuvem.velocityX = -6;
